...
class TicTacToeEnv(gym.Env):
    ...
    def step(self, move):
        ...
        if move not in emptySqares:
            self.reset()
            return self._get_observation(), -1.0, True, {reason: 'bad move, loose!'}
        self.game.move(move)
        
        winner = self.winner()
        if winner == 'T':
            self.reset()
            return self._get_observation(), 0.01, True, {reason: 'tie!'}
        
        if winner:
            self.reset()
            return self._get_observation(), 1.0, True, {reason: 'you win!'}
        ...
        if winner:
            self.reset()
            return self._get_observation(), -0.99, True, {reason: 'you loose!'}
